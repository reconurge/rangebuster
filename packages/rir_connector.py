...
class RiRConnector(Connector):
    def __init__(self, output_file, keywords, strict, source, db_file):
        ...
        self.db_file = os.path.join(CACHE_PATH, db_file)
        
    def download_database(self, url, output_filename):
        if not os.path.exists(output_filename):
            logger.info(f'{RESET}Downloading {BOLD}{self.source}{RESET} database. This may take a little while.')
            try:
                ...
                with open(output_filename, 'wb') as file:
                    for data in response.iter_content(chunk_size=block_size):
                        file.write(data)
                logger.info(f'Downloaded {output_filename} successfully.')
    ...
    def search_database(self):
        output_grep_file = os.path.join(CACHE_PATH, f'{self.db_file}.txt')
        ...
        if self.db_file.endswith("gz"):
            ...
            if self.keywords.endswith('.txt'):
                ...
                keyword_file_new = os.path.join(CACHE_PATH, f"keywords_{str(datetime.now().strftime('%d%m%y%H%M%S'))}.txt")
